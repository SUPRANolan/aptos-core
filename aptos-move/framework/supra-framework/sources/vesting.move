...
module supra_framework::vesting {
    ...
    fun withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<SupraCoin> {
        ...
        let withdrawn_coins = coin::balance<SupraCoin>(contract_address);
        let contract_signer = &get_vesting_account_signer_internal(vesting_contract);
        coin::withdraw<SupraCoin>(contract_signer, withdrawn_coins)
    }
    ...
    public entry fun test_end_to_end(
        ...
        distribute(contract_address);
        let shareholder_1_bal = coin::balance<SupraCoin>(shareholder_1_address);
        let shareholder_2_bal = coin::balance<SupraCoin>(shareholder_2_address);
        ...
        assert!(coin::balance<SupraCoin>(shareholder_1_address) == shareholder_1_bal + pending_distribution / 4, 0);
        assert!(coin::balance<SupraCoin>(shareholder_2_address) == shareholder_2_bal + pending_distribution * 3 / 4, 1);
        // Withdrawal address receives the left-over dust of 1 coin due to rounding error.
        assert!(coin::balance<SupraCoin>(withdrawal_address) == 1, 0);
        ...
        let previous_bal = coin::balance<SupraCoin>(withdrawal_address);
        admin_withdraw(admin, contract_address);
        assert!(coin::balance<SupraCoin>(withdrawal_address) == previous_bal + withdrawn_amount, 0);
    }
    ...
    #[test(supra_framework = @0x1, admin = @0x123, shareholder = @0x234, operator = @0x345)]
    public entry fun test_unlock_rewards_should_pay_commission_first(
        ...
        let shareholder_address = signer::address_of(shareholder);
        setup(supra_framework, &vector[admin_address, shareholder_address, operator_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        assert!(coin::balance<SupraCoin>(shareholder_address) == staker_rewards + 1, 0);
        assert!(coin::balance<SupraCoin>(operator_address) == commission - 1, 1);
    }
    ...
    #[test(supra_framework = @0x1, admin = @0x123, operator = @0x345)]
    public entry fun test_update_operator_with_same_commission(
        ...
        let admin_address = signer::address_of(admin);
        let operator_address = signer::address_of(operator);
        setup(supra_framework, &vector[admin_address, @11, operator_address]);
        let contract_address = setup_vesting_contract(
    ...
    public entry fun test_set_beneficiary_for_operator(
        ...
        setup(supra_framework, &vector[admin_address, shareholder_address, operator_address1, beneficiary_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        assert!(operator_commission_percentage(contract_address) == 0, 0);
        let stake_pool_address = stake_pool_address(contract_address);
        ...
        assert!(coin::balance<SupraCoin>(operator_address1) == 0, 1);
        assert!(coin::balance<SupraCoin>(beneficiary_address) == expected_commission, 1);
        let old_beneficiay_balance = coin::balance<SupraCoin>(beneficiary_address);
    ...
    public entry fun test_reset_beneficiary(
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        assert!(coin::balance<SupraCoin>(@11) == vested_amount, 0);
        assert!(coin::balance<SupraCoin>(@12) == 0, 1);
    }
