...
module supra_framework::stake {
    ...
    /// power.
    public(friend) fun on_new_epoch() acquires StakePool, SupraCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees {
        let validator_set = borrow_global_mut<ValidatorSet>(@supra_framework);
        let config = staking_config::get();
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@supra_framework);
    ...
    public fun configure_allowed_validators(supra_framework: &signer, accounts: vector<address>) acquires AllowedValidators {
    ...
    public entry fun test_multiple_validators_join_and_leave(
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@supra_framework).pending_inactive, 0).addr == validator_2_address, 0);
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@supra_framework).pending_active, 0).addr == validator_3_address, 0);
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@supra_framework).active_validators, 0).config.consensus_pubkey == pk_1_bytes, 0);
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@supra_framework).active_validators, 0).config.consensus_pubkey == pk_1b_bytes, 0);
    ...
    #[test_only]
    public fun end_epoch() acquires StakePool, SupraCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet, ValidatorFees {
    ...
    fun test_distribute_validator_fees(
        ...
        features::change_feature_flags(supra_framework, vector[COLLECT_AND_DISTRIBUTE_GAS_FEES], vector[]);
